...
public class ImageDownloader {
   public static void main(String[] args) {
      // Make the call to receive the packets
      Packet[] p = PacketReceiver.receivePackets();
      Comparator<Packet> comparator = new PacketComparator();
      // Sort the packets using merge sort.
      PacketSorter.mergeSort(p, comparator);
      // Sort the packets using selection sort.
    	//PacketSorter.selectionSort(p, comparator);
      String imageOutputFilename = "finalImage.jpg";
      //Make the correct call to render the images
      PacketRenderer.renderImage(p, imageOutputFilename);
   }
